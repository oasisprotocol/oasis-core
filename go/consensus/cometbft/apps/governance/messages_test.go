...
func TestChangeParameters(t *testing.T) {
	...
	t.Run("invalid proposal", func(t *testing.T) {
		...
		require.EqualError(err, "cometbft/governance: failed to type assert change parameters proposal")
	})
	...
	t.Run("empty changes", func(t *testing.T) {
		...
		_, err := app.changeParameters(ctx, &proposal, true)
		require.EqualError(err, "cometbft/governance: failed to validate consensus parameter changes: consensus parameter changes should not be empty")
	})
