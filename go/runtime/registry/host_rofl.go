...
type roflHostHandler struct {
	...
	clientSrv     runtimeClient.RuntimeClient
...
func newSubHandlerROFL(parent *runtimeHostHandler, cr host.CompositeRuntime) (host.RuntimeHandler, error) {
	clientSrv, err := parent.env.GetRuntimeRegistry().Client()
	if err != nil {
		return nil, err
	}
	...
	return &roflHostHandler{
		...
		cr:            cr,
		clientSrv:     clientSrv,
		eventNotifier: newROFLEventNotifier(parent.runtime, clientSrv, logger),
		logger:        logger,
	}, nil
}
...
func (rh *roflHostHandler) handleHostSubmitTx(
	...
	case true:
		...
		rsp, err := rh.clientSrv.SubmitTxMeta(ctx, submitRq)
		switch {
		case err != nil:
			return nil, err
	...
	default:
		...
		err := rh.clientSrv.SubmitTxNoWait(ctx, submitRq)
		if err != nil {
			return nil, err
		}
...
func newROFLEventNotifier(runtime Runtime, clientSrv runtimeClient.RuntimeClient, logger *logging.Logger) *roflEventNotifier {
		...
		case cmd := <-en.cmdCh:
			...
			case cmd.attachRuntime != nil:
				...
				default:
					...
					blkCh, blkSub, err = en.clientSrv.WatchBlocks(ctx, en.runtime.ID())
					if err != nil {
