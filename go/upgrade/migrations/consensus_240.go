...
func (h *Handler240) ConsensusUpgrade(privateCtx interface{}) error {
	...
	case abciAPI.ContextEndBlock:
		...
		if err != nil {
			return fmt.Errorf("failed to load governance parameters: %w", err)
		}
		govParams.StakeThreshold = 60 // 68 -> 60

		if err = govState.SetConsensusParameters(abciCtx, govParams); err != nil {
			return fmt.Errorf("failed to set governance parameters: %w", err)
		}
	default:
