...
type newCreator = func(dir string, ndb dbApi.NodeDB) (Creator, error)
...
func FuzzCreateRestore(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64, n uint16, chunkSize uint64) {
		...
		for _, creator := range creators {
			...
			if err := testCreateRestoreRoundtrip(ctx, backend1, backend2, creator, n, chunkSize, rnd); err != nil {
...
func testCreateRestoreRoundtrip(ctx context.Context, backend1, backend2 dbApi.Factory, creator newCreator, n uint16, chunkSize uint64, rnd *rand.Rand) error {
	...
	root, err := populateDB(ctx, []dbApi.NodeDB{ndb1}, testNs, n, rnd)
	if err != nil {
		return fmt.Errorf("populate db with random entries: %v", err)
	}

	// Create a checkpoint.
	fc, err := creator(filepath.Join(dir, "checkpoints"), ndb1)
	if err != nil {
...
func testDeterministicOutput(ctx context.Context, creator newCreator, n uint16, chunkSize uint64, rnd *rand.Rand) error {
	dir, err := os.MkdirTemp("", "mkvs.checkpoint.CreateDeterministic")
	if err != nil {
		return fmt.Errorf("create new temporary dir: %v", err)
	}
	defer os.RemoveAll(dir)
	var ndbs []dbApi.NodeDB
	for i := 0; i < 3; i++ {
		backend := db.Backends[rnd.Intn(len(db.Backends))]
		// Create node database.
		cfg1 := &dbApi.Config{
			DB:           filepath.Join(dir, fmt.Sprintf("db-%d", i)),
			Namespace:    testNs,
			MaxCacheSize: 16 * 1024 * 1024,
		}
		...
		defer ndb.Close()
		ndbs = append(ndbs, ndb)
	}

	// Populate all node databases with same random entries.
	root, err := populateDB(ctx, ndbs, testNs, n, rnd)
	if err != nil {
	...
	result.Empty()
	for i, ndb := range ndbs {
		// Create a checkpoint.
		fc, err := creator(filepath.Join(dir, fmt.Sprintf("db-%d/checkpoints", i)), ndb)
		if err != nil {
			return fmt.Errorf("create new file creator: %v", err)
		}
		cp, err := fc.CreateCheckpoint(ctx, root, chunkSize)
...
// populateDB populates all provided databases with same n random entries.
func populateDB(ctx context.Context, ndbs []dbApi.NodeDB, ns common.Namespace, n uint16, rnd *rand.Rand) (node.Root, error) {
	trees := make([]mkvs.Tree, 0, len(ndbs))
	...
	for i, tree := range trees {
		...
		err = ndbs[i].Finalize([]node.Root{root})
		if err != nil {
