...
use ekiden_stake_base::{AmountType, EscrowAccountIdType, EscrowAccountIterator, EscrowAccountStatus, StakeEscrowBackend, StakeStatus};
...
impl StakeEscrowBackend for StakeClient {
    ...
    fn transfer(&self, sender: B256, destination_address: B256, value: AmountType) -> BoxFuture<(bool)> {
    ...
    fn approve(&self, msg_sender: B256, spender_address: B256, value: AmountType) -> BoxFuture<bool> {
    ...
    fn approve_and_call(&self, msg_sender: B256, spender_address: B256, value: AmountType, extra_data: Vec<u8>) -> BoxFuture<bool> {
    ...
    fn allowance(&self, owner: B256, spender: B256) -> BoxFuture<AmountType> {
        ...
        match self.0.allowance_async(&request) {
            Ok(f) => Box::new(
                f.map(|response| AmountType::from_little_endian(response.get_remaining()))
                    .map_err(|error| Error::new(error.description()))),
            Err(error) => Box::new(future::err(Error::new(error.description()))),
        }
    }
    ...
    fn list_active_escrows_iterator(&self, owner: B256) -> BoxFuture<EscrowAccountIterator> {
        ...
        match self.0.list_active_escrows_iterator_async(&request) {
            Ok(f) => Box::new(f.map_err(|error| Error::new(error.description())).and_then(
                move |response| Ok(EscrowAccountIterator::new(
    ...
    fn list_active_escrows_get(&self, it: EscrowAccountIterator) -> BoxFuture<(EscrowAccountStatus, EscrowAccountIterator)> {
        assert!(it.has_next);
    ...
    fn take_and_release_escrow(
        ...
        match self.0.take_and_release_escrow_async(&request) {
            Ok(f) => Box::new(
                f.map(|response| AmountType::from_little_endian(response.get_amount_taken()))
                    .map_err(|error| Error::new(error.description())),
            ),
            Err(error) => Box::new(future::err(Error::new(error.description()))),
        }
