...
func (app *Application) completeStateSync(ctx *api.Context) (any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cometbft/governance: couldn't get pending upgrades: %w", err)
	}
...
func (app *Application) changeParameters(ctx *api.Context, msg any, apply bool) (any, error) {
	...
	if !ok {
		return nil, fmt.Errorf("cometbft/governance: failed to type assert change parameters proposal")
	}
	...
	if err := cbor.Unmarshal(proposal.Changes, &changes); err != nil {
		return nil, fmt.Errorf("cometbft/governance: failed to unmarshal consensus parameter changes: %w", err)
	}
	...
	if err = changes.Apply(params); err != nil {
		return nil, fmt.Errorf("cometbft/governance: failed to apply consensus parameter changes: %w", err)
	}
	if err = params.SanityCheck(); err != nil {
		return nil, fmt.Errorf("cometbft/governance: failed to validate consensus parameters: %w", err)
	}

	// Apply changes.
	if apply {
		if err = state.SetConsensusParameters(ctx, params); err != nil {
			return nil, fmt.Errorf("cometbft/governance: failed to update consensus parameters: %w", err)
		}
	}
