...
func (r *RegistryHelpers) runRegistryRuntimeSubcommand(
	...
	if runtime.Kind == registry.KindCompute {
		args = append(args,
			...
			"--"+cmdRegRt.CfgExecutorGroupSize, strconv.FormatUint(uint64(runtime.Executor.GroupSize), 10),
			"--"+cmdRegRt.CfgExecutorGroupBackupSize, strconv.FormatUint(uint64(runtime.Executor.GroupBackupSize), 10),
			"--"+cmdRegRt.CfgExecutorAllowedStragglers, strconv.FormatUint(uint64(runtime.Executor.AllowedStragglers), 10),
			...
			"--"+cmdRegRt.CfgStorageGroupSize, strconv.FormatUint(uint64(runtime.Storage.GroupSize), 10),
			"--"+cmdRegRt.CfgStorageMinWriteReplication, strconv.FormatUint(uint64(runtime.Storage.MinWriteReplication), 10),
