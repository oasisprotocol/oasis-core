...
var (
	...
	//
	// The only reason why this is needed is because CometBFT only gives you
...
func (s *ImmutableState) ConsensusParameters(ctx context.Context) (*registry.ConsensusParameters, error) {
	...
	if raw == nil {
		return nil, errors.New("cometbft/registry: expected consensus parameters to be present in app state")
	}
...
func (s *MutableState) SetNode(ctx context.Context, existingNode, node *node.Node, signedNode *node.MultiSignedNode) error { //nolint: gocyclo
	...
	if existingNode != nil && !existingNode.Consensus.ID.Equal(node.Consensus.ID) {
		...
		address := []byte(tmcrypto.PublicKeyToCometBFT(&existingNode.Consensus.ID).Address())
	...
	address := []byte(tmcrypto.PublicKeyToCometBFT(&node.Consensus.ID).Address())
