...
func (app *stakingApplication) initParameters(ctx *abciAPI.Context, state *stakingState.MutableState, st *staking.Genesis) error {
	...
	if err := state.SetConsensusParameters(ctx, &st.Parameters); err != nil {
		return fmt.Errorf("cometbft/staking: failed to set consensus parameters: %w", err)
	}
	return nil
}
...
func (app *stakingApplication) initCommonPool(ctx *abciAPI.Context, st *staking.Genesis, totalSupply *quantity.Quantity) error {
	...
	if err := totalSupply.Add(&st.CommonPool); err != nil {
		...
		return fmt.Errorf("cometbft/staking: failed to add common pool: %w", err)
	}
...
func (app *stakingApplication) initLastBlockFees(ctx *abciAPI.Context, st *staking.Genesis, totalSupply *quantity.Quantity) error {
	...
	if err := totalSupply.Add(&st.LastBlockFees); err != nil {
		...
		return fmt.Errorf("cometbft/staking: failed to add last block fees: %w", err)
	}
	...
	if err := st.CommonPool.Add(&st.LastBlockFees); err != nil {
		return fmt.Errorf("cometbft/staking: failed to add block fees to common pool")
	}
...
func (app *stakingApplication) initGovernanceDeposits(ctx *abciAPI.Context, state *stakingState.MutableState, st *staking.Genesis, totalSupply *quantity.Quantity) error {
	...
	if err := totalSupply.Add(&st.GovernanceDeposits); err != nil {
		...
		return fmt.Errorf("cometbft/staking: failed to add governance deposits: %w", err)
	}
	if err := state.SetGovernanceDeposits(ctx, &st.GovernanceDeposits); err != nil {
		...
		return fmt.Errorf("cometbft/staking: failed to set governance deposits state")
	}
	return nil
}
...
func (app *stakingApplication) initLedger(
	...
	for addr, acct := range st.Ledger {
		if acct == nil {
			return fmt.Errorf("cometbft/staking: genesis ledger account %s is nil", addr)
		}
		if !addr.IsValid() {
			return fmt.Errorf("cometbft/staking: genesis ledger account %s: address is invalid", addr)
		}
		if !acct.General.Balance.IsValid() {
			...
			return fmt.Errorf("cometbft/staking: invalid genesis general balance for account %s", addr)
		}
		if !acct.Escrow.Active.Balance.IsValid() {
			...
			return fmt.Errorf("cometbft/staking: invalid genesis active escrow balance for account %s", addr)
		}
		if !acct.Escrow.Debonding.Balance.IsValid() {
			...
			return fmt.Errorf("cometbft/staking: invalid genesis debonding escrow balance for account %s", addr)
		}
		...
		if err := totalSupply.Add(&acct.Escrow.Debonding.Balance); err != nil { //nolint:gosec
			...
			return fmt.Errorf("cometbft/staking: failed to add debonding escrow balance: %w", err)
		}

		if err := state.SetAccount(ctx, addr, acct); err != nil {
			return fmt.Errorf("cometbft/staking: failed to set account %s: %w", addr, err)
		}
	}
	return nil
}
...
func (app *stakingApplication) initTotalSupply(
	...
	if totalSupply.Cmp(&st.TotalSupply) != 0 {
		...
		return fmt.Errorf("cometbft/staking: total supply mismatch (expected: %s actual: %s)", st.TotalSupply, totalSupply)
	}
	...
	if err := state.SetTotalSupply(ctx, totalSupply); err != nil {
		return fmt.Errorf("cometbft/staking: failed to set total supply: %w", err)
	}
	...
	for escrowAddr, delegations := range st.Delegations {
		...
		for delegatorAddr, delegation := range delegations {
			if !delegatorAddr.IsValid() {
				return fmt.Errorf(
					"cometbft/staking: failed to set genesis delegation from %s to %s: delegator address is invalid",
					delegatorAddr, escrowAddr,
				)
			}
			if delegation == nil {
				return fmt.Errorf("cometbft/staking: genesis delegation from %s to %s is nil",
					delegatorAddr, escrowAddr,
				)
			}
			if err := delegationShares.Add(&delegation.Shares); err != nil { //nolint:gosec
				...
				return fmt.Errorf("cometbft/staking: failed to add delegation shares to %s from %s: %w",
					escrowAddr, delegatorAddr, err,
				)
			}
			if err := state.SetDelegation(ctx, delegatorAddr, escrowAddr, delegation); err != nil {
				return fmt.Errorf("cometbft/staking: failed to set delegation to %s from %s: %w",
					escrowAddr, delegatorAddr, err,
				)
			}
		}
		...
		if err != nil {
			return fmt.Errorf("cometbft/staking: failed to fetch escrow account %s: %w", escrowAddr, err)
		}
		if acc.Escrow.Active.TotalShares.Cmp(delegationShares) != 0 {
			...
			return fmt.Errorf("cometbft/staking: total active shares mismatch for account %s: actual: %s (expected: %s)",
...
func (app *stakingApplication) initDebondingDelegations(ctx *abciAPI.Context, state *stakingState.MutableState, st *staking.Genesis) error {
	for escrowAddr, delegators := range st.DebondingDelegations {
		...
		for delegatorAddr, delegations := range delegators {
			if !delegatorAddr.IsValid() {
				return fmt.Errorf(
					"cometbft/staking: failed to set genesis debonding delegation from %s to %s: delegator address is invalid",
					delegatorAddr, escrowAddr,
				)
			}
			for idx, delegation := range delegations {
				...
				if delegation == nil {
					return fmt.Errorf(
						"cometbft/staking: genesis debonding delegation from %s to %s with index %d is nil",
				...
				if err := debondingShares.Add(&delegation.Shares); err != nil { //nolint:gosec
					...
					return fmt.Errorf("cometbft/staking: failed to add debonding delegation shares to %s from %s index %d: %w",
				...
				if err := state.SetDebondingDelegation(ctx, delegatorAddr, escrowAddr, delegation.DebondEndTime, delegation); err != nil {
					return fmt.Errorf("cometbft/staking: failed to set debonding delegation to %s from %s index %d: %w",
						escrowAddr, delegatorAddr, idx, err,
					)
				}
			}
		...
		if acc.Escrow.Debonding.TotalShares.Cmp(debondingShares) != 0 {
			...
			return fmt.Errorf("cometbft/staking: total debonding shares mismatch for account %s: actual: %s (expected: %s)",
