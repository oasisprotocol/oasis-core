...
func New(
	...
	genesisBlocks := make(map[signature.MapKey]*api.Block)
	...
	if genesisBlocksFilename != "" {
		...
		for _, genesisBlock := range pbGenesisBlocks.GenesisBlocks {
			...
			if err := id.UnmarshalBinary(genesisBlock.GetContractId()); err != nil {
				return nil, err
			}
			var block api.Block
			if err := block.FromProto(genesisBlock.GetBlock()); err != nil {
				return nil, err
			}
			genesisBlocks[id.ToMapKey()] = &block
		}
	}
