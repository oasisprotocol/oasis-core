...
func (n *Node) worker() { //nolint: gocyclo
	...
	for _, comp := range bundleRegistry.Components(n.Runtime.ID()) {
		policyProvider := n.quotePolicyProvider(comp)
		if err := n.ProvisionHostedRuntimeComponent(comp, policyProvider); err != nil {
	...
	for {
		...
		case compNotify := <-compCh:
			...
			case compNotify.Added != nil:
				...
				policyProvider := n.quotePolicyProvider(compNotify.Added)
				if err := n.ProvisionHostedRuntimeComponent(compNotify.Added, policyProvider); err != nil {
...
func (n *Node) quotePolicyProvider(comp *bundle.ExplodedComponent) *quotePolicyProvider {
	return &quotePolicyProvider{
		cs:           n.Consensus,
		runtimeID:    n.Runtime.ID(),
		isRonl:       comp.ID().IsRONL(),
		version:      comp.Version,
		useKMAPolicy: n.willRegisterComputeRuntime,
	}
}
