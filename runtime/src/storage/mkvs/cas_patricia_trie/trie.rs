...
use crate::{common::crypto::hash::Hash, storage::CAS};
...
/// A merkle patricia tree backed by CAS.
...
impl PatriciaTrie {
    ...
    /// Construct a new merkle patricia tree backed by given CAS.
    ...
    fn derive_key(key: &[u8]) -> NibbleVec {
        NibbleVec::from_key(Hash::digest_bytes(key).as_ref())
    }

    /// Return pointer to root node.
    fn get_root_pointer(&self, root: Option<Hash>) -> NodePointer {
        match root {
            Some(ref root) if root.is_empty() => NodePointer::Null,
    ...
    fn get_path_by_pointer(&self, path: NibbleVec, pointer: NodePointer) -> Option<Vec<u8>> {
        match pointer {
            ...
            NodePointer::Pointer(pointer) => {
                let node = self.cas.get(pointer).expect("failed to fetch from cas");
    ...
    /// Lookup key.
    pub fn get(&self, root: Option<Hash>, key: &[u8]) -> Option<Vec<u8>> {
        ...
        if node.is_embeddable() {
            ...
            NodePointer::Pointer(
                self.cas
                    ...
                    .expect("failed to insert to cas"),
            )
        }
    }
    ...
    /// Insert key.
    pub fn insert(&self, root: Option<Hash>, key: &[u8], value: &[u8]) -> Hash {
        let path = Self::derive_key(key);
    ...
    /// Remove key.
    pub fn remove(&self, root: Option<Hash>, key: &[u8]) -> Option<Hash> {
        if root.is_none() {
            return None;
        }
    ...
    /// ```
    pub fn dump(&self, root: Option<Hash>, max_depth: usize) {
    ...
    pub fn stats(&self, root: Option<Hash>, max_depth: usize) -> TrieStats {
...
mod test {
    ...
    use crate::storage::cas::memory::MemoryCAS;
    ...
    fn test_basic_ops() {
        ...
        assert_eq!(
            new_root,
            Hash::from("0xe6772f39648c428810ed68426da21155dabc79a62f2ab7c9800766ef0414c6cd")
        );
        ...
        assert_eq!(
            new_root,
            Hash::from("0xc747dbbb1752fc6755f23799d373df9c3f991ba76a44eac05fdddcc5f6821a73")
        );
        ...
        assert_eq!(
            new_root,
            Hash::from("0xeddda540b0c7386cc0c7503e1cea5e27eea86ab1ec98d67cbcfb7b554e7b1bf0")
        );
    ...
    fn test_empty_hash() {
        ...
        assert_eq!(tree.get(Some(Hash::empty_hash()), b"foo"), None);
        let new_root = tree.insert(Some(Hash::empty_hash()), b"foo", b"bar");
        assert_eq!(tree.get(Some(new_root), b"foo"), Some(b"bar".to_vec()));
        assert_eq!(
            new_root,
            Hash::from("0xe6772f39648c428810ed68426da21155dabc79a62f2ab7c9800766ef0414c6cd")
