...
func TestGenerateStatus(t *testing.T) {
	...
	sigInitResponseSecure, err := api.SignInitResponse(rakSigner, &initResponse)
	require.NoError(t, err, "SignInitResponse")
	...
	t.Run("No nodes", func(t *testing.T) {
		...
		newStatus := app.generateStatus(ctx, runtimes[0], uninitializedStatus, nil, nodes[0:6], params, epoch)
		require.Equal(uninitializedStatus, newStatus, "key manager committee should be empty")

		newStatus = app.generateStatus(ctx, runtimes[0], initializedStatus, nil, nodes[0:6], params, epoch)
	...
	t.Run("One node", func(t *testing.T) {
		...
		newStatus := app.generateStatus(ctx, runtimes[0], uninitializedStatus, nil, nodes[6:7], params, epoch)
		require.Equal(expStatus, newStatus, "node 6 should form the committee if key manager not initialized")

		newStatus = app.generateStatus(ctx, runtimes[0], expStatus, nil, nodes[6:7], params, epoch)
		...
		expStatus.Nodes = nil
		newStatus = app.generateStatus(ctx, runtimes[0], initializedStatus, nil, nodes[6:7], params, epoch)
	...
	t.Run("Multiple nodes", func(t *testing.T) {
		...
		newStatus := app.generateStatus(ctx, runtimes[0], uninitializedStatus, nil, nodes, params, epoch)
		...
		newStatus = app.generateStatus(ctx, runtimes[0], uninitializedStatus, nil, reverse(nodes), params, epoch)
		...
		newStatus = app.generateStatus(ctx, runtimes[0], initializedStatus, nil, nodes, params, epoch)
		...
		newStatus = app.generateStatus(ctx, runtimes[1], initializedStatus, nil, nodes, params, epoch)
