...
func New(cmd *cobra.Command, timeSource epochtime.Backend, conReg registry.ContractRegistry, entReg registry.EntityRegistry, beacon beacon.Backend) (api.Backend, error) {
	backend, _ := cmd.Flags().GetString(cfgBackend)
	switch strings.ToLower(backend) {
	case trivial.BackendName:
		return trivial.New(timeSource, conReg, entReg, beacon), nil
	default:
