...
type Registry interface {
	...
	Indexer(runtimeID common.Namespace) (*history.BlockIndexer, error)
...
func (r *runtimeRegistry) Indexer(runtimeID common.Namespace) (*history.BlockIndexer, error) {
	r.RLock()
	defer r.RUnlock()

	indexer, ok := r.indexers[runtimeID]
	if !ok {
		return nil, fmt.Errorf("runtime/registry: indexer not found for runtime %s", runtimeID)
	}
	return indexer, nil
}
