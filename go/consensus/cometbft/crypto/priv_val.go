...
func (pv *privVal) GetPubKey() (cmtcrypto.PubKey, error) {
	return PublicKeyToCometBFT(&pv.PublicKey), nil
}
...
func (pv *privVal) SignVote(chainID string, vote *cmtproto.Vote) error {
	...
	if err != nil {
		return fmt.Errorf("cometbft/crypto: failed to check vote H/R/S: %w", err)
	}
	...
	if equivocation {
		if bytes.Equal(signBytes, pv.SignBytes) {
			...
			err = fmt.Errorf("cometbft/crypto: conflicting vote")
		}
		return err
	}

	sig, err := pv.signer.ContextSign(cometbftSignatureContext, signBytes)
	if err != nil {
		return fmt.Errorf("cometbft/crypto: failed to sign vote: %w", err)
	}
...
func (pv *privVal) SignProposal(chainID string, proposal *cmtproto.Proposal) error {
	...
	equivocation, err := pv.CheckHRS(height, round, step)
	if err != nil {
		return fmt.Errorf("cometbft/crypto: failed to check proposal H/R/S: %w", err)
	}
	...
	if equivocation {
		if bytes.Equal(signBytes, pv.SignBytes) {
			...
			err = fmt.Errorf("cometbft/crypto: conflicting proposal")
		}
		return err
	}
	...
	sig, err := pv.signer.ContextSign(cometbftSignatureContext, signBytes)
	if err != nil {
		return fmt.Errorf("cometbft/crypto: failed to sign proposal: %w", err)
	}
	if err = pv.update(height, round, step, signBytes, sig); err != nil {
		return err
	}
...
func (pv *privVal) save() error {
	...
	if err = tempfile.WriteFileAtomic(pv.filePath, b, 0o600); err != nil {
		return fmt.Errorf("cometbft/crypto: failed to save private validator file: %w", err)
	}

	return nil
}

// LoadOrGeneratePrivVal loads or generates a CometBFT PrivValidator for an
...
func LoadOrGeneratePrivVal(baseDir string, signer signature.Signer) (cmttypes.PrivValidator, error) {
	...
	if err == nil {
		if err = json.Unmarshal(b, &pv); err != nil {
			return nil, fmt.Errorf("cometbft/crypto: failed to parse private validator file: %w", err)
		}

		// CometBFT doesn't do this, but it's cheap insurance.
		if !signer.Public().Equal(pv.PublicKey) {
			return nil, fmt.Errorf("cometbft/crypto: public key mismatch, state corruption?: %w", err)
		}
	} else if os.IsNotExist(err) {
		pv.PublicKey = signer.Public()

		if err = pv.save(); err != nil {
			return nil, fmt.Errorf("cometbft/crypto: failed to save newly generate key: %w", err)
		}
	} else {
		return nil, fmt.Errorf("cometbft/crypto: failed to load private validator file: %w", err)
	}
