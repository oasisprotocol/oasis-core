...
func onEvidenceRuntimeEquivocation(
	...
	if err != nil {
		...
		return fmt.Errorf("cometbft/roothash: failed to get node by id %s: %w", pk, roothash.ErrInvalidEvidence)
	}
	...
	default:
		return fmt.Errorf("cometbft/roothash: failed to lookup node: %w", err)
	}
...
func onRuntimeIncorrectResults(
	...
	for _, pk := range discrepancyCausers {
		...
		if err = totalSlashed.Add(slashed); err != nil {
			return fmt.Errorf("cometbft/roothash: totalSlashed.Add(slashed): %w", err)
		}
...
func distributeSlashedFunds(
	...
	if err := runtimeAccReward.Mul(quantity.NewFromUint64(runtimePercentage)); err != nil {
		return fmt.Errorf("cometbft/roothash: runtimeAccReward.Mul: %w", err)
	}
	if err := runtimeAccReward.Quo(quantity.NewFromUint64(uint64(100))); err != nil {
		return fmt.Errorf("cometbft/roothash: runtimeAccReward.Quo(100): %w", err)
	}
	...
	if _, err := stakeState.TransferFromCommon(ctx, runtimeAddr, runtimeAccReward, false); err != nil {
		return fmt.Errorf("cometbft/roothash: failed transferring reward to %s: %w", runtimeAddr, err)
	}
	...
	if err := otherReward.Sub(runtimeAccReward); err != nil {
		return fmt.Errorf("cometbft/roothash: remainingReward.Sub(runtimeAccReward): %w", err)
	}
	if err := otherReward.Quo(quantity.NewFromUint64(uint64(len(otherAddresses)))); err != nil {
		return fmt.Errorf("cometbft/roothash: remainingReward.Quo(len(discrepancyResolvers)): %w", err)
	}

	for _, addr := range otherAddresses {
		if _, err := stakeState.TransferFromCommon(ctx, addr, otherReward, true); err != nil {
			return fmt.Errorf("cometbft/roothash: failed transferring reward to %s: %w", addr, err)
		}
