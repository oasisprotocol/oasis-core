...
var (
	...
	RegistryCLI scenario.Scenario = &registryCLIImpl{
		e2eImpl: *newE2eImpl("registry-cli"),
	}
)
...
func (r *registryCLIImpl) testEntityAndNode(childEnv *env.Env, cli *cli.Helpers) error {
	...
	for _, val := range r.net.Validators() {
...
func (r *registryCLIImpl) listEntities(childEnv *env.Env) ([]signature.PublicKey, error) {
	...
	args := []string{
		...
		"--" + grpc.CfgAddress, "unix:" + r.net.Validators()[0].SocketPath(),
	}
	out, err := cli.RunSubCommandWithOutput(childEnv, r.logger, "list", r.net.Config().NodeBinary, args)
	if err != nil {
...
func (r *registryCLIImpl) initEntity(childEnv *env.Env, entDir string) (*entity.Entity, error) {
	...
	out, err := cli.RunSubCommandWithOutput(childEnv, r.logger, "entity-init", r.net.Config().NodeBinary, args)
	if err != nil {
...
func (r *registryCLIImpl) updateEntity(childEnv *env.Env, nodes []*node.Node, nodeGenesisFiles []string, entDir string) (*entity.Entity, error) {
	...
	out, err := cli.RunSubCommandWithOutput(childEnv, r.logger, "entity-update", r.net.Config().NodeBinary, args)
	if err != nil {
...
func (r *registryCLIImpl) listNodes(childEnv *env.Env) ([]signature.PublicKey, error) {
	...
	out, err := cli.RunSubCommandWithOutput(childEnv, r.logger, "node-list", r.net.Config().NodeBinary, args)
	if err != nil {
...
func (r *registryCLIImpl) isRegistered(childEnv *env.Env, nodeName, nodeDataDir string) error {
	...
	out, err := cli.RunSubCommandWithOutput(childEnv, r.logger, "is-registered", r.net.Config().NodeBinary, args)
	if err != nil {
...
func (r *registryCLIImpl) initNode(childEnv *env.Env, ent *entity.Entity, entDir string, dataDir string) (*node.Node, error) {
	...
	runInitNode := func() (*node.Node, error) {
		...
		out, err = cli.RunSubCommandWithOutput(childEnv, r.logger, "init-node", r.net.Config().NodeBinary, args)
		if err != nil {
...
func (r *registryCLIImpl) genRegisterEntityTx(childEnv *env.Env, nonce int, txPath string, entDir string) error {
	...
	if out, err := cli.RunSubCommandWithOutput(childEnv, r.logger, "gen_register", r.net.Config().NodeBinary, args); err != nil {
...
func (r *registryCLIImpl) genDeregisterEntityTx(childEnv *env.Env, nonce int, txPath string, entDir string) error {
	...
	if out, err := cli.RunSubCommandWithOutput(childEnv, r.logger, "gen_deregister", r.net.Config().NodeBinary, args); err != nil {
