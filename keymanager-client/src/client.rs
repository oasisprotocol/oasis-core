...
use ekiden_runtime::{common::sgx::avr::MrEnclave, protocol::Protocol, rak::RAK, rpc::session};
...
impl RemoteClient {
    ...
    pub fn new_runtime(
        ...
        Self::new(RpcClient::new_runtime(
            session::Builder::new()
...
impl KeyManagerClient for RemoteClient {
    ...
    fn get_or_create_keys(&self, ctx: Context, contract_id: ContractId) -> BoxFuture<ContractKey> {
        ...
        Box::new(
            self.inner
                .rpc_client
                .get_or_create_keys(ctx, contract_id)
                .and_then(move |keys| {
    ...
    fn get_public_key(
        ...
        Box::new(
            self.inner
                .rpc_client
                .get_public_key(ctx, contract_id)
    ...
    fn get_long_term_public_key(
        ...
        Box::new(
            self.inner
                .rpc_client
                .get_long_term_public_key(ctx, contract_id)
                .and_then(move |key| match key {
                    Some(key) => {
