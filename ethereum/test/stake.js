...
contract("Ethereum Stake test", async (accounts) => {
    ...
    it("should handle adding escrow", async () => {
        let instance = await Stake.deployed();
        ...
        result = await instance.addEscrow(BN_escrow_amount);
        events.stopWatching();
        ...
        for (var tx_hash in contract_events) {
            ...
            assert.equal(args.owner, accounts[0],
                        "EscrowAdd event owner wrong");
            assert(args.amount_added.eq(BN_escrow_amount),
                  "EscrowAdd event amount wrong");
        }

        // fetchEscrowAmount
        var fetch_amount = await instance.fetchEscrowAmount(accounts[0]);
        ...
        var a0_bal = await instance.balanceOf.call(accounts[0]);
        assert(expected.eq(a0_bal),
               "post-add available balance for 1st account wrong");
        ...
        assert(expected.eq(bal), 
               "post-add StakeStatus stake wrong");
        assert(BN_escrow_amount.eq(escrowed),
               "post-add StakeStatus escrowed wrong");
    });
