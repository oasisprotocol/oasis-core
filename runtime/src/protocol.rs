...
impl Protocol {
    ...
    fn io_write(self: &Arc<Protocol>) {
        ...
        while let Ok(message @ Message { id, message_type, .. }) = self.outgoing_rx.recv() {
            if let Err(error) = self.write_message(message) {
                warn!(self.logger, "Failed to write message"; "err" => %error);
                // Handle write failure by sending error response.
    ...
    fn handle_write_failure(
        ...
        match message_type {
            ...
            MessageType::Response => {
                ...
                // Try to send the error response; if this also fails, just log it.
                if let Err(retry_error) = self.write_message(error_response) {
                    warn!(self.logger, "Failed to write message"; 
                          "original_err" => %error, "retry_err" => %retry_error);
                }
            }
            _ => warn!(self.logger, "Write failure for invalid message type"; "err" => %error),
        }
    }
