// Package statesync defines the background worker responsible for syncing,
// prunning and initializing the runtime state using the relevant p2p protocol
// clients.
package statesync
...
//  2. Prunning the state as specified by the configuration.
...
// Run watches CometBFT new block events for the given runtime.
//
// If checkpointer is enabled it starts a background worker that triggers
// runtime state checkpoint for every consensus checkpoint.
func (w *Worker) Run(ctx context.Context) error { // nolint: gocyclo
	...
	return w.sync(ctx, cachedLastRound)
}
	...
	for {
		...
		if applyNext {
			...
			if err != nil {
				...
				continue // Suggestion: make a nested for loop and break here.
			}
...
func (w *Worker) fetchDiff(ctx context.Context, fetchCh chan<- *fetchedDiff, round uint64, prevRoot, thisRoot storageApi.Root) {
	result := &fetchedDiff{
		fetched:  false,
