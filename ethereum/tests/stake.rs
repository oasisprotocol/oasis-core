...
fn stake_integration() {
    ...
    // Connect to truffle.
    let (handle, transport) = WebSocket::new("ws://localhost:9545").expect("WebSocket creation should work");
    ...
    let total_supply = stake.get_total_supply().wait().expect("totalSupply should work");
    ...
    let stake_status = stake.get_stake_status(oasis).wait().expect("getStakeStatus should work");
    debug!("total_stake = {}", stake_status.total_stake);
    debug!("escrowed = {}", stake_status.escrowed);
    assert_eq!(stake_status.total_stake, total_supply, "initial total_stake should be total supply");
    assert_eq!(stake_status.escrowed, AmountType::from(0), "initial amount escrowed should be zero");
    ...
    assert_eq!(oasis_addr, DEVELOPMENT_ADDRESS, "truffle framework test data bad?");
    ...
    let oasis_balance = stake.balance_of(oasis).wait().expect("balanceOf should go through");
    debug!("oasis balance = {}", oasis_balance);
    assert_eq!(oasis_balance, total_supply);

    let alice_balance = stake.balance_of(alice).wait().expect("balanceOf should go through");
    ...
    let alice_balance = stake.balance_of(alice).wait().expect("balanceOf(alice) should work");
    assert_eq!(alice_balance, oasis_to_alice_transfer_amt, "post-transfer Alice balance wrong");

    let oasis_balance = stake.balance_of(oasis).wait().expect("balanceOf(oasis) should work");
    assert_eq!(oasis_balance, total_supply - oasis_to_alice_transfer_amt, "post-transfer Oasis balance wrong");

    let alice_approval = AmountType::from(100);
    let b = stake.approve(oasis, alice, alice_approval).wait().expect("approval should go through");
    ...
    let bob_balance = stake.balance_of(bob).wait().expect("balanceOf(bob) should work");
    assert_eq!(bob_balance, oasis_to_bob_transfer_amt);

    let expected_oasis_balance = oasis_balance - oasis_to_bob_transfer_amt;
    let oasis_balance = stake.balance_of(oasis).wait().expect("balanceOf(oasis) should work");
    assert_eq!(oasis_balance, expected_oasis_balance, "post-approved-transfer oasis balance wrong");

    let expected_remaining_allowance = alice_approval - oasis_to_bob_transfer_amt;
    let remaining_allowance = stake.allowance(oasis, alice).wait().expect("allowance(oasis, alice) should work");
    assert_eq!(remaining_allowance, expected_remaining_allowance, "post-approved-transfer allowance wrong");

    let excessive_transfer_amount = remaining_allowance + AmountType::from(1);
    match stake.transfer_from(alice, oasis, bob, excessive_transfer_amount)
        .wait() {
            Ok(b) => { assert!(!b, "excessive transfer_from did not revert, and returned TRUE"); }
            Err(e) => { debug!("excessive transfer_from aborted correctly: {}", e.description()); }
        }

    let burn_quantity = AmountType::from(10_000);
    let b = stake.burn(oasis, burn_quantity).wait().expect("burn(oasis, 10_000) should work");
    assert!(b, "burn(oasis, 10_000) should succeed and return true");

    let expected_oasis_balance = oasis_balance - burn_quantity;
    let oasis_balance = stake.balance_of(oasis).wait().expect("balanceOf(oasis) should continue to work");
    assert_eq!(oasis_balance, expected_oasis_balance, "post-burn oasis balance wrong");

    let expected_total_supply = total_supply - burn_quantity;
    let total_supply = stake.get_total_supply().wait().expect("total_supply after burn should work");
    ...
    let b = stake.approve(oasis, carol, carol_burn_approve_amount).wait().expect("carol burn approval should work");
    assert!(b, "carol approval should return true, even when exceeds total supply");

    let carol_balance = stake.balance_of(carol).wait().expect("balanceOf(carol) should work");
    assert_eq!(carol_balance, AmountType::from(0));

    let carol_burns_oasis_amount = AmountType::from(20_000);
    let b = stake.burn_from(carol, oasis, carol_burns_oasis_amount).wait().expect("burn_from should work");
    ...
    assert_eq!(oasis_balance, expected_oasis_balance, "post-burn_from oasis balance wrong");
