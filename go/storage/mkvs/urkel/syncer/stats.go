...
func (c *StatsCollector) GetSubtree(ctx context.Context, root node.Root, id node.ID, maxDepth node.DepthType) (*Subtree, error) {
	c.SubtreeFetches++
	return c.rs.GetSubtree(ctx, root, id, maxDepth)
}

func (c *StatsCollector) GetPath(ctx context.Context, root node.Root, key node.Key, startDepth node.DepthType) (*Subtree, error) {
	c.PathFetches++
