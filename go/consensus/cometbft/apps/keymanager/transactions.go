...
func (app *keymanagerApplication) updatePolicy(
	...
	newStatus := app.generateStatus(ctx, kmRt, oldStatus, nodes, regParams, epoch)
...
//
// Key managers support forward-secret ephemeral secrets which are never encrypted with
// SGX sealing key nor stored in the enclave's cold storage. These secrets are generated
// by the enclaves themselves for the next epoch only and published encrypted in the consensus.
...
// does not effect the performance, as key managers store in memory only the last few secrets,
